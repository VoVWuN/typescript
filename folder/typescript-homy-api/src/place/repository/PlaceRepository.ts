...
export class PlaceRepository {
  ...
  public async find(filter: Filter) {
    if (
      filter.coordinates[0] !== 59.9386 ||
      filter.coordinates[1] !== 30.3141
    ) {
      return [];
    }
    ...
    const dateRange = DateHelper.generateDateRange(
      filter.startDate,
      filter.endDate
    );
    places = places.filter((place) => {
  ...
  public async book(place: Place, dateRange: Date[]): Promise<Place> {
    if (!this.checkIfPlaceAreAvailableForDates(place, dateRange)) {
      throw new BadRequest(
        `Place ${place.id} is not available for dates ${dateRange.join(",")}.`
      );
    }
  ...
  private checkIfPlaceAreAvailableForDates(
    place: Place,
    dateRange: Date[]
  ): boolean {
